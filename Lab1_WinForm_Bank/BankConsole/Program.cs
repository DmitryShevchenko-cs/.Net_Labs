        ...
        while (!exit)
        {
            ...
            switch (choice)
            {
                ...
                case "5":
                    ...
                    await bankService.SendMoneyAsync(user.Id, cardNumber, decimal.Parse(money!));
                    break;
                case "6":
                    Console.Clear();
                    Console.WriteLine("Count of nearest ATMs");
                    var count = Console.ReadLine();
                    Console.WriteLine(atmService.ToString(await atmService.FindNearestATMs(random.Next(1, 10), int.Parse(count))));
                    break;
